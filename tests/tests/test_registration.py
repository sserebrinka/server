...
with open(DATA_DIR / "valid_users_register.json", encoding="utf-8") as f:
    valid_users_register = json.load(f)

with open(DATA_DIR / "invalid_users_register.json", encoding="utf-8") as f:
    invalid_users_register = json.load(f)

@pytest.mark.registration
@pytest.mark.parametrize("user", valid_users_register)
def test_registration_success(base_url, user):
    ...
    assert data["user"]["age"] == user["age"], "Возраст в ответе не совпадает с переданным"

@pytest.mark.registration
@pytest.mark.parametrize("user", invalid_users_register)
def test_register_fail(base_url, user):
    response = requests.post(f"{base_url}/auth/register", json=user)
    data = response.json()
    status_code = response.status_code
    ...
    if user["email"] == "example1@gmail.com" and user["password"] == "12345" and user["age"] == 0:
